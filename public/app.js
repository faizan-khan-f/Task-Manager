...
// Generate or retrieve a persistent unique anonymous user identifier for this browser
...
// Helper function to include the anonymous ID header in all requests
function getHeaders() {
  return {
    ...
    .map((todo) => {
      ...
      return `
        <div class="group bg-slate-800/40 hover:bg-slate-800/80 border border-slate-700/60 transition duration-200 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 ${
          ...
          <div class="flex items-center gap-2 self-end sm:self-center">
            <button onclick="startEdit('${todo._id}')" class="px-3 py-1.5 text-xs bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-lg transition">Edit</button>
            <button onclick="deleteTodo('${todo._id}')" class="px-3 py-1.5 text-xs bg-rose-500/10 hover:bg-rose-500/20 text-rose-400 rounded-lg transition">Delete</button>
          </div>
        </div>
      `;
    })
